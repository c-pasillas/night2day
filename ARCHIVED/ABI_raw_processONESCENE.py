...
def parse_filename_viirs(path):
    ...
    return {'filename': name, 'channels': n[0], 'satellite': n[1],'start': start, 'end': end, 'path': str(path), "datetime": n[2] + '_' + n[3]}
...
def parse_filename_abi(path):
    ...
    return {'filename': name, 'channels': n[0], 'satellite': n[2], 'sat_and_start': f'{n[2]} {start}','start': start, 'end': end, 'path': str(path)}
...
def group_by_datetime(files):
    """Given a list of parsed filenames match them up by corresponding datetime (so that the files can be colocated later by the Scene library. Filenames that don't have a paired file are returned separately."""
    def f(d, file):
