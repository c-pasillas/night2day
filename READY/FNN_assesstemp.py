...
def load_data(raw, norm, ML):
    ...
    M16 = raw["M16"]
    DNB = raw['DNB']
    DNB_FMN = norm['DNB_FMN']
    DNB_logFMN = norm['DNB_log_FMN']
    normML = ML['MLtruth']
...
def assessment(args):
    ...
    print("I loaded the DNB/Channels norms case")
    ML = np.load(args.ML_path)
    print("I loaded the ML_DNB case")
    
    data_dic = load_data(raw, norm, ML)#currently a dict of arrays 
    print('Done with load_data(raw, norm, ML)')
