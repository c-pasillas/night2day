...
def parse_time(time_str):
    ...
    #return int(datetime.datetime.strptime(time_str, format_abi))# if len(time_str) /= 14 else format_abi)
...
def pack_case(args):
    """Scan for abi files, pair them by datetime,
    ...
    nc_dict = group_abi_by_time_sat(abi_dir)

    paired = pair_with_ncs(nc_dict)
    print(paired)
    paired_sorted = sorted(paired)#, key=lambda h5_ncs: h5_ncs[0]['start'])
    for nc_list in paired_sorted:
        nc_list = sorted(nc_list, key=lambda nc: (nc[0]['start']))
        nc_first = nc_list[0]
        #print(f'nc_list first {nc_first['start']}')
        #import sys
        #sys.exit()
