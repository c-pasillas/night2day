...
def normalize(db_path: Path):
    log.info("I'm in normalize!")
    with np.load(db_path.parent / 'case.npz') as f:
        case = dict(f)
    norm = normalize_case(case)
    log.info(f'dims {norm["DNB"].shape}')
    np.savez(db_path.parent / 'case_norm.npz', **norm)
